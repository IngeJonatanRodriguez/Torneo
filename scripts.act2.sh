...
# Modifica un equipo existente (utilizando el _id del documento):
db.equipos.update(
  { "_id": ObjectId("663a167003b3201d8a824612") },
  { $set: { "pais": "Argentina" } }
)

# Eliminar un equipo  (utilizando el _id del documento):
db.equipos.deleteOne({ "_id": ObjectId("663a167003b3201d8a824612") })

#Inscribir, modificar y eliminar deportistas:
...
db.deportistas.insert({
  ...
  "posicion": "Delantero",
  "equipo": {
    "nombre": "Equipo Anthropic FC",
    "pais": "Estados Unidos"
  }
})

#Modificar un deportista  (utilizando el _id del documento):
db.deportistas.update(
  { "_id": ObjectId("64573f7a8b030c3d9c7f3457") },
  { $set: { "posicion": "Volante" } }
)

#Eliminar un deportista  (utilizando el _id del documento):
db.deportistas.deleteOne({ "_id": ObjectId("64573f7a8b030c3d9c7f3457") })

#modificar y eliminar encuentros deportivos:
#Inserta un nuevo encuentro deportivo:
...
#Modifica un encuentro deportivo existente (utilizando el _id del documento):
